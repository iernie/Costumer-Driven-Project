...
The risk factor in the test for the P3P parser, UNIT-02, was considered as the test was run ten times on different policies. It was attempted to choose different policies to parse, from few to many cases, and with different contents in the cases. All of these ten tests were run successfully, with no bugs in the parser or missing elements. The reason this test took so long from start to finish, is that the expiry date was not parsed at all for any policy. This was a very low priority issue so it did not get fixed quickly.
