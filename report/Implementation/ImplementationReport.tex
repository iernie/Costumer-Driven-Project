...
One of the specific ways, of computing the confidence, that is most relative to this case, is based that actions can be grouped together by some attribute. Let's consider that action is binary, simple yes, no, then we can group the set, given by KNN, into two factions. If for example the action we take is yes. Then we can calculate the confidence by counting the number of objects that have the same action as the decision, and divide it by total number of cases, producing a fraction between 0 and 1. If the confidence is 0 then there's no close neighbours found by KNN, and the result is most likely wrong. If the confidence is 1 then all of the close neighbours have the same action and the result is most likely right. 
But what if the closest neighbour is the right one, and the others in given set from KNN are wrong, how can we then use the distance to help us calculate the confidence? One of the methods is to use \textit{reverse distance}. It is just to count the cases, the difference is that we count the values of one divided by distance from target, object with an action we want to calculate confidence to, to the each object in the given set. Given that distance to all objects is known. This way the object that is closer to the target affects the confidence more.
