...
% Comment from customer: It might happen that the report needs some more background to the problem that you are trying to solve? The current version says little about what "personal data" really is, why sharing personal data is a problem (from the end-user's perspective), the background to machine readable privacy policies, etc. I've attached a paper that we wrote in the beginning of our project that explains some of this. But maybe you should ask you NTNU supervisor if such information is necessary in the final report before you spend any time on this. 



\subsection{Current Privacy Technology}\label{privTech}
As discussed in T{\o}ndel et al. (2011), there is a disproportion between the average Internet user's concern for privacy and the actual control he has over private information. While most websites provide a privacy policy, these tend to very long documents written in a obscure "legalesque" language, intended  for protecting the websites' interests than those of users. To aid this problem, machine readable standards such as P3P have been introduced. P3P seeks to compress the information contained in the privacy policy in an XML document that can be parsed and summarized by computer programs. 
